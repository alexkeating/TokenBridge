...
contract PaymentBridgeFactory is Initializable{
    ...
    function __PaymentBridgeFactory_init_unchained(address _template) internal initializer {
        template = _template;
    }
    // init
    // @dev embeds linearized calls to all parent initializers
    function __PaymentBridgeFactory_init(address _template) internal initializer {
        __PaymentBridgeFactory_init_unchained(_template);
    }
    // initialize
    /// @dev Initializes factory contract using a minimal proxy pattern (EIP-1167)
    function initialize(address _template) public{
        __PaymentBridgeFactory_init(_template);
    }
    ...
    function createPaymentBridge(bytes calldata _initData) external {
